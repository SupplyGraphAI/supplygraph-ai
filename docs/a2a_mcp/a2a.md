...
| Quick Examples | [quick_example.md](./quick_example.md) |
| Agent Library | [../agents/index.md](../agents/index.md) |
| Agent API | [../agent-api/agent-api.md](../agent-api/agent-api.md) |
...
## 13. Client Options (Python)

### Recommended — Standard A2A

Use the official **[A2A Python SDK](https://pypi.org/project/a2a-sdk/)** (`pip install a2a-sdk httpx`, Python 3.10+).

See [quick_example.md](./quick_example.md) for a minimal `create_client` → `send_message` → `get_task` workflow.

### Recommended — Standard MCP

Use the official **[MCP Python SDK](https://pypi.org/project/mcp/)** (`pip install mcp`).

See [quick_example.md](./quick_example.md) for `call_tool_as_task` → `poll_task` → `get_task_result`.

### Optional — Agent API SDK

**[supplygraphai_a2a_sdk](https://github.com/SupplyGraphAI/supplygraphai_a2a_sdk)** wraps the traditional **Agent API** (`run` / `status` / `results` at `/api/v1/agents/...`). Use this only if you prefer the REST-style interface over standard A2A or MCP.
